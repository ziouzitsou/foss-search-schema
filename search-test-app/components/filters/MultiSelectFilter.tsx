...
export default function MultiSelectFilter({
  ...
  const getIPIcon = (ipRating: string) => {
    // Simple IP rating icons (could be enhanced with actual icons)
      ...
      {searchable && facets.length > 10 && (
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-8 pr-2 py-1 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
          />
        </div>
      )}
          ...
          filteredFacets.map((facet) => {
            ...
            return (
              <label
                ...
                className="flex items-center gap-2 text-sm cursor-pointer hover:bg-gray-50 p-1 rounded group"
              >
                <input
                  type="checkbox"
                  checked={isChecked}
                ...
                {/* Value label */}
                <span className={`flex-1 ${isChecked ? 'font-medium' : ''}`}>
                  {facet.filter_value}
                </span>
